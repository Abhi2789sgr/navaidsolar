...
$dataSql = "SELECT `id`,`dev_id`,`name`,`ward`,`panchayat`,`block`,`district`,`project`,`luminary_qr`,`battery_qr`,`panel_qr`,`lat`,`lng`,`remark`,`updated_by`,`time`,`active` FROM _f_device".$condition;
...
echo 'IMEI'."\t".'Pole Id'."\t".'Project Name'."\t".'District'."\t".'Block'."\t".'Panchayat'."\t".'Ward'."\t".'Luminary QR'."\t".'Battery QR'."\t".'Panel QR'."\t".'Latitude'."\t".'Longitude'."\t".'Remark'."\t".'Installed By'."\t".'Date'."\t".'Time'."\t".'Approved Status'."\n";
...
foreach ($devData as $dRow){
	...
	$csvData = $dRow['dev_id']."\t".$dRow['name']."\t".$projeName."\t".$distName."\t".$blockName."\t".$panchName."\t".$wardName."\t".$dRow['luminary_qr']."\t".$upperBattery."\t".$dRow['panel_qr']."\t".$dRow['lat']."\t".$dRow['lng']."\t".$ucRemark."\t".$urInstaller."\t".$dateTimeArr[0]."\t".$dateTimeArr[1]."\t".$approvedStatus."\n".$csvData;
