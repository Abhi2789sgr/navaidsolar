...
if (true) //isGet("imei") )
{
	if ($_GET['filter'] == '0') {
		...
		$sql = "SELECT _g_data_latest.*, _f_device.name, _f_device.updated_by FROM _g_data_latest INNER JOIN _f_device ON    _f_device.dev_id=_g_data_latest.device where _g_data_latest.device IN ({$sql2}) ORDER BY _g_data_latest.time DESC";
	} else if ($_GET['filter'] == '2') {
		if ($_GET['project_id'] != "") {

			//for admin and user
			$sql = "SELECT _g_data_latest.*, _f_device.name, _f_device.updated_by FROM _g_data_latest INNER JOIN _f_device ON    _f_device.dev_id=_g_data_latest.device where _f_device.dev_id={$_GET['imei']} AND _f_device.project={$_GET['project_id']} ORDER BY _g_data_latest.time DESC";
			// echo $sql;
			// exit;
		} else {
			//for master
			$sql = "SELECT _g_data_latest.*, _f_device.name, _f_device.updated_by FROM _g_data_latest INNER JOIN _f_device ON    _f_device.dev_id=_g_data_latest.device where _f_device.dev_id={$_GET['imei']} ORDER BY _g_data_latest.time DESC";
		...
		if (isset($GET['project_id'])) {
			...
			//for admin and user
			$sql = "SELECT _g_data_latest.*, _f_device.name, _f_device.updated_by FROM _g_data_latest INNER JOIN _f_device ON    _f_device.dev_id=_g_data_latest.device where _f_device.name='{$_GET['pole_id']}' AND _f_device.project={$_GET['project_id']} ORDER BY _g_data_latest.time DESC";
		} else {

			//for master
			$sql = "SELECT _g_data_latest.*, _f_device.name, _f_device.updated_by FROM _g_data_latest INNER JOIN _f_device ON    _f_device.dev_id=_g_data_latest.device where _f_device.name='{$_GET['pole_id']}' ORDER BY _g_data_latest.time DESC";
		}
	}
